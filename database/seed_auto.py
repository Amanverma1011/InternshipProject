#!/usr/bin/env python3
"""Quick seed for local dev — creates master + templates + company settings."""
import sys, os
...
with app.app_context():
    ...
    db.session.commit()
    print('Company settings: OK')

    # Master user
    existing = User.query.filter_by(role='MASTER').first()
    if existing:
        print(f'Master already exists: {existing.username}')
    else:
        master = User(
            name=MASTER_NAME, username=MASTER_USERNAME,
        ...
        db.session.add(Template(name='Ongrid v1', system_type='ONGRID', version=1,
                                is_active=True, html_file='pdf/ongrid.html', created_by=master.id))
        db.session.add(Template(name='Hybrid v1', system_type='HYBRID', version=1,
                                is_active=True, html_file='pdf/hybrid.html', created_by=master.id))
        db.session.commit()
        print(f'Master created: username={MASTER_USERNAME}  password={MASTER_PASSWORD}')
        print('Templates: Ongrid v1, Hybrid v1')
